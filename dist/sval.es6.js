(function(global, factory) {
    ...
    switch (node.operator) {
      ...
      case 'delete':
        if (arg.type === 'MemberExpression') {
          const variable = MemberExpression(arg, scope, {
            getVar: true
          });
          return variable.del();
  ...
  function ForInStatement(node, scope) {
    for (const value in evaluate(node.right, scope)) {
      const result = ForXHandler$1(node, scope, {
        value
      });
      if (result === BREAK) {
        break;
    ...
    for (let i = 0; i < node.elements.length; i++) {
      ...
      if (hoist) {
        ...
        if (kind) {
          ...
          const variable = Identifier(element, scope, {
            getVar: true
          });
          variable.set(feed[i]);
  ...
  function RestElement(node, scope, options = {}) {
    ...
    if (hoist) {
      ...
      if (kind) {
        ...
        const variable = Identifier(arg, scope, {
          getVar: true
        });
        variable.set(feed);
      }
  ...
  function AssignmentPattern(node, scope, options = {}) {
    const {
      kind = 'var', hoist = false, onlyBlock = false, feed = evaluate(node.right, scope)
    } = options;
    const left = node.left;
    if (hoist) {
      if (onlyBlock || kind === 'var') {
  ...
  function* UnaryExpression$1(node, scope) {
    ...
    switch (node.operator) {
      ...
      case 'typeof':
        if (arg.type === 'Identifier') {
          return typeof(yield* Identifier$1(arg, scope, {
            throwErr: false
          }));
        } else {
      ...
      case 'delete':
        if (arg.type === 'MemberExpression') {
          const variable = yield* MemberExpression$1(arg, scope, {
            getVar: true
          });
          return variable.del();
        } else if (arg.type === 'Identifier') {
  ...
  function* UpdateExpression$1(node, scope) {
    ...
    let variable;
    if (arg.type === 'Identifier') {
      variable = yield* Identifier$1(arg, scope, {
        getVar: true
      });
    } else if (arg.type === 'MemberExpression') {
      variable = yield* MemberExpression$1(arg, scope, {
        getVar: true
      });
    } else {
      throw new SyntaxError('Unexpected token');
  ...
  function* AssignmentExpression$1(node, scope) {
    ...
    let variable;
    if (left.type === 'Identifier') {
      variable = yield* Identifier$1(left, scope, {
        getVar: true,
        throwErr: false
      });
      if (!variable) {
      ...
      variable = yield* MemberExpression$1(left, scope, {
        getVar: true
      });
    } else {
      const value = yield* evaluate$1(node.right, scope);
      return yield* pattern$2(left, scope, {
        feed: value
      });
    }
    const value = yield* evaluate$1(node.right, scope);
  ...
  function* MemberExpression$1(node, scope, options = {}) {
    ...
    let object;
    if (node.object.type === 'Super') {
      object = yield* Super$1(node.object, scope, {
        getProto: true
      });
    } else {
  ...
  function* CallExpression$1(node, scope) {
    ...
    let object;
    if (node.callee.type === 'MemberExpression') {
      object = yield* MemberExpression$1(node.callee, scope, {
        getObj: true
      });
      if (node.callee.optional && object == null) {
        return undefined;
  ...
  function* CatchClause$1(node, scope) {
    return yield* BlockStatement$1(node.body, scope, {
      invasived: true
    });
  }
      ...
      if (hoist) {
        if (onlyBlock || kind === 'var') {
          if (property.type === 'Property') {
            ...
            yield* RestElement$1(property, scope, {
              kind,
              hoist,
              onlyBlock
            });
    ...
    for (let i = 0; i < node.elements.length; i++) {
      ...
      if (hoist) {
        ...
        if (kind) {
          ...
          const variable = yield* Identifier$1(element, scope, {
            getVar: true
          });
          variable.set(feed[i]);
          result.push(variable.get());
        ...
        yield* RestElement$1(element, scope, {
          kind,
          feed: feed.slice(i)
        });
      } else {
  ...
  function* RestElement$1(node, scope, options = {}) {
    ...
    if (hoist) {
      ...
      if (kind) {
        ...
        const variable = yield* Identifier$1(arg, scope, {
          getVar: true
        });
        variable.set(feed);
      }
  ...
  function* AssignmentPattern$1(node, scope, options = {}) {
    const {
      kind = 'var', hoist = false, onlyBlock = false, feed = yield* evaluate$1(node.right, scope)
    } = options;
    const left = node.left;
    if (hoist) {
      if (onlyBlock || kind === 'var') {
  ...
  function* VariableDeclaration$1(node, scope, options = {}) {
    for (let i = 0; i < node.declarations.length; i++) {
      yield* VariableDeclarator$1(node.declarations[i], scope, assign({
        kind: node.kind
      }, options));
    }
  }
  ...
  function* ClassBody$1(node, scope, options = {}) {
    ...
    for (let i = 0; i < node.body.length; i++) {
      const def = node.body[i];
      if (def.type === 'MethodDefinition') {
        yield* MethodDefinition$1(def, scope, {
          klass,
          superClass
        });
      }
  ...
  function* hoist(block, scope, options = {}) {
    ...
    for (let i = 0; i < block.body.length; i++) {
      ...
      if (statement.type === 'FunctionDeclaration') {
        ...
        yield* VariableDeclaration$1(statement, scope, {
          hoist: true,
          onlyBlock: true
        });
      } else if (!onlyBlock) {
    ...
    switch (statement.type) {
      ...
      case 'ForOfStatement':
        if (statement.left.type === 'VariableDeclaration') {
          yield* VariableDeclaration$1(statement.left, scope, {
            hoist: true
          });
        }
      case 'ForStatement':
        if (statement.type === 'ForStatement' && statement.init.type === 'VariableDeclaration') {
          yield* VariableDeclaration$1(statement.init, scope, {
            hoist: true
          });
        }
      case 'WhileStatement':
    ...
    const tmpFunc = function*(...args) {
      ...
      for (let i = 0; i < params.length; i++) {
        ...
        if (param.type === 'Identifier') {
          ...
          yield* RestElement$1(param, subScope, {
            kind: 'var',
            feed: args.slice(i)
          });
        } else {
          yield* pattern$2(param, subScope, {
            kind: 'var',
            feed: args[i]
          });
        }
  ...
  function* createClass(node, scope) {
    ...
    yield* ClassBody$1(node.body, scope, {
      klass,
      superClass
    });
  ...
  function* ForXHandler(node, scope, options) {
    ...
    if (left.type === 'VariableDeclaration') {
      yield* VariableDeclaration$1(left, subScope, {
        feed: value
      });
    } else if (left.type === 'Identifier') {
  ...
  function ForXHandler$1(node, scope, options) {
    ...
    if (left.type === 'VariableDeclaration') {
      ...
      const variable = Identifier(left, scope, {
        getVar: true
      });
      variable.set(value);
    } else {
