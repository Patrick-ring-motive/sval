    ...
    if (hoist) {
      if (onlyBlock || kind === 'var') {
        if (property.type === 'Property') {
          ...
          yield* RestElement(property, scope, {
            kind,
            hoist,
            onlyBlock
          })
  ...
  for (let i = 0; i < node.elements.length; i++) {
    ...
    if (hoist) {
      ...
      if (kind) {
        ...
        const variable = yield* Identifier(element, scope, {
          getVar: true
        })
        variable.set(feed[i])
      ...
      yield* RestElement(element, scope, {
        kind,
        feed: feed.slice(i)
      })
    } else {
...
export function* RestElement(node: acorn.RestElement, scope: Scope, options: PatternOptions = {}) {
  ...
  if (hoist) {
    ...
    if (kind) {
      ...
      // If kind is undefined, it's a statement
      const variable = yield* Identifier(arg, scope, {
        getVar: true
      })
      variable.set(feed)
    }
...
export function* AssignmentPattern(node: acorn.AssignmentPattern, scope: Scope, options: PatternOptions = {}) {
  const {
    kind = 'var', hoist = false, onlyBlock = false, feed = yield* evaluate(node.right, scope)
  } = options
